...
const Add = () => {
  ...
  // FIX: ID ke zariye sahi item delete karne ke liye filter use kiya
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-gray-50 min-h-screen">
      {/* Input Section */}
      <div className="flex flex-col sm:flex-row items-center gap-4 bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-8">
        <textarea 
          type="text" 
          value={Inputlist} 
          ...
          rows={2} 
          className="w-full p-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-500 resize-none text-gray-700"
        />
        <button 
          onClick={handleData}
          className="w-full sm:w-auto bg-yellow-500 hover:bg-yellow-600 text-white font-semibold px-6 py-3 rounded-lg transition duration-200 shadow-md whitespace-nowrap"
        >
          Add Task
        </button>
      </div>
      ...
      {/* Cards Grid Section */}
      {/* 1 column mobile par, 2 columns tablets par, aur 3 columns laptops par bnenge */}
      <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6'>
        {Lissdata.map((item, index) => {
          return (
            <div 
              key={index} 
              className='flex flex-col justify-between bg-yellow-50 border border-yellow-200 p-5 rounded-xl transition-all duration-300 hover:scale-105 hover:shadow-lg min-h-[160px]'
            >
              {/* Card Header */}
              <div className='flex justify-between items-center border-b border-yellow-200 pb-2 mb-3'>
                ...
                <button 
                  onClick={() => Deltedata(index)} // Idhar index pass kiya
